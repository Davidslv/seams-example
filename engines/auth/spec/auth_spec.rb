...
# is run from the host root (`bundle exec rspec engines/auth/spec`)
# or from inside the engine. The host's spec_helper does not require us.
# Rails must load before lib/auth.rb because engine.rb references
# Rails::Engine.
require "rails"
...
RSpec.describe Auth do
  it "defines a VERSION constant" do
    expect(Auth::VERSION).to match(/\A\d+\.\d+\.\d+\z/)
  end
end
