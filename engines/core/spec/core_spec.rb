...
# is run from the host root (`bundle exec rspec engines/core/spec`)
# or from inside the engine. The host's spec_helper does not require us.
# Rails must load before lib/core.rb because engine.rb references
# Rails::Engine.
require "rails"
...
RSpec.describe Core do
  it "defines a VERSION constant" do
    expect(Core::VERSION).to match(/\A\d+\.\d+\.\d+\z/)
  end
end
